...
class AgnosticGridOut(object):
    ...
    def __getattr__(self, item):
        if not self.delegate._file:
            raise pymongo.errors.InvalidOperation(
                "You must call MotorGridOut.open() before accessing the %s property"
                % item
            )
    ...
    def open(self):
        ...
        """
        return self._framework.chain_return_value(
            self._ensure_file(), self.get_io_loop(), self
        )
...
""",
    ...
    def __init__(self, root_collection, delegate=None, session=None, **kwargs):
        ...
        if not isinstance(root_collection, collection_class):
            raise TypeError(
                "First argument to MotorGridIn must be MotorCollection, not %r"
                % root_collection
            )
...
class AgnosticGridFSBucket(object):
    ...
    download_to_stream_by_name = AsyncCommand(
        doc=docstrings.gridfs_download_to_stream_by_name_doc
    )
    open_download_stream = AsyncCommand(
        doc=docstrings.gridfs_open_download_stream_doc
    ).wrap(gridfs.GridOut)
    ...
    open_upload_stream = DelegateMethod(
        doc=docstrings.gridfs_open_upload_stream_doc
    ).wrap(gridfs.GridIn)
    ...
    upload_from_stream_with_id = AsyncCommand(
        doc=docstrings.gridfs_upload_from_stream_with_id_doc
    )
