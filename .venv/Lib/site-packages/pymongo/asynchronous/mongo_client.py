...
class AsyncMongoClient(common.BaseObject, Generic[_DocumentType]):
    ...
    def _init_based_on_options(
        ...
        self._topology_settings = TopologySettings(
            ...
            topology_id=self._topology_settings._topology_id
            if self._topology_settings
            else None,
        )
    ...
    async def _close_cursor_now(
        ...
        if not isinstance(cursor_id, int):
            raise TypeError(
                f"cursor_id must be an instance of int, not {type(cursor_id)}"
            )
...
class _MongoClientErrorHandler:
    ...
    def __init__(
        ...
        if not isinstance(client, AsyncMongoClient):
            ...
            if not any(
                cls.__name__ == "AsyncMongoClient" for cls in type(client).__mro__
            ):
                raise TypeError(
