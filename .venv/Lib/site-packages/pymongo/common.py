...
def validate_integer(option: str, value: Any) -> int:
    ...
    raise TypeError(
        f"Wrong type for {option}, value must be an integer, not {type(value)}"
    )
        ...
        for key, value in value.items():  # noqa: B020
            if isinstance(value, str):
                ...
                if not isinstance(value, OIDCCallback):
                    raise ValueError(
                        f"callback must be an OIDCCallback object, not {type(value)}"
                    )
                props[key] = value
            else:
                raise ValueError(
...
def validate_auto_encryption_opts_or_none(option: Any, value: Any) -> Optional[Any]:
    ...
    if not isinstance(value, AutoEncryptionOpts):
        raise TypeError(
            f"{option} must be an instance of AutoEncryptionOpts, not {type(value)}"
        )
